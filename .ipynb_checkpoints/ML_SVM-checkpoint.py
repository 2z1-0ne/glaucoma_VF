...
grid_search = GridSearchCV(SVR(), param_grid, cv =3, n_jobs=-1, verbose = 4)
...
mean_squared = pd.DataFrame(mean_squared, columns=['MSE'])
pearson = pd.DataFrame(pearson, columns=['Pearson'])
...
mean_squared.to_csv('mean_squared_SVM.csv', index=False)
pearson.to_csv('pearson_SVM.csv', index=False)
